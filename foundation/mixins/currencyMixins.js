...
const currencyMixins = {
    methods: {
        mxSetFiatMaskForInput(value, fiatDecimals){
            ...
            v = parseFloat(v);
            v = v.toFixed(x);
            return new Intl.NumberFormat('pt-BR', { minimumFractionDigits: x }).format(v);
        },
        ...
        mxAllowFiatFormatOnly(e){
            if((e.charCode >= 48 && e.charCode <= 57 || (e.charCode == 44 || e.charCode == 46)) == false){
                e.preventDefault();
            }
        },
        mxAllowCryptoValuesOnly(e){
            if((e.charCode >= 48 && e.charCode <= 57 || (e.charCode == 44)) == false){
                e.preventDefault();
            }
        },
        ...
        mxClearMasks(value, raw){
            ...
            if(dot == -1){
                ...
                const integerValue = value.substring(0, dot),
                    decimals = value.substring(dot + 1);
